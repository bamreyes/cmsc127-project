...
async function seed() {
  try {
    ...
    await pool.query("DROP TABLE IF EXISTS traffic_violations;");
    await pool.query("DROP TABLE IF EXISTS vehicles;");
    await pool.query("DROP TABLE IF EXISTS vehicle_registrations;");
    ...
    await pool.query(`
      CREATE TABLE traffic_violations (
        ...
        CONSTRAINT fk_tv_driver FOREIGN KEY (license_number) REFERENCES drivers (license_number) ON DELETE SET NULL,
        CONSTRAINT fk_tv_vehicle FOREIGN KEY (plate_number) REFERENCES vehicles (plate_number) ON DELETE SET NULL
      )
    `);
    ...
    console.log("Seeding traffic violations...");
    const violations = [
